...
const MAX_TRACK_ENDPOINT = `${DISCOVERY_PROVIDER}/latest/track`
const MAX_COLLECTION_ENDPOINT = `${DISCOVERY_PROVIDER}/latest/playlist`
const MAX_USER_ENDPOINT = `${DISCOVERY_PROVIDER}/latest/user`
...
const fetchTracks = async (latestId, maxIterations) => {
  const maxId = await getStoppingPoint(MAX_TRACK_ENDPOINT)
  const { items, latest } = await fetch(TRACKS_ENDPOINT, latestId, maxId, maxIterations)
  return {
...
const fetchCollections = async (latestId, maxIterations) => {
  const maxId = await getStoppingPoint(MAX_COLLECTION_ENDPOINT)
  const { items, latest } = await fetch(COLLECTIONS_ENDPOINT, latestId, maxId, maxIterations)
  return {
...
const fetchUsers = async (latestId, maxIterations) => {
  const maxId = await getStoppingPoint(MAX_USER_ENDPOINT)
  const { items, latest } = await fetch(USERS_ENDPOINT, latestId, maxId, maxIterations)
  return {

...
const PREVIOUSLY_UNLISTED_TRACKS_ENDPOINT = `${DISCOVERY_PROVIDER}/previously_unlisted/track`
const PREVIOUSLY_PRIVATE_COLLECTIONS_ENDPOINT = `${DISCOVERY_PROVIDER}/previously_private/playlist`
...
const fetchTracks = async (latestId, maxIterations) => {
  ...
  if (items.length > 0) {
    const previouslyUnlistedTracks = await fetchPreviouslyHidden(
      latestId,
      TRACKS_ENDPOINT,
      PREVIOUSLY_UNLISTED_TRACKS_ENDPOINT
    )
...
const fetchCollections = async (latestId, maxIterations) => {
  ...
  if (items.length > 0) {
    const previouslyUnlistedTracks = await fetchPreviouslyHidden(
      ...
      PREVIOUSLY_PRIVATE_COLLECTIONS_ENDPOINT
    )
    items.push(...previouslyUnlistedTracks)
  }

...
const getPreviouslyUnlistedTracksEndpoint = discprov => `${discprov}/previously_unlisted/track`
const getPreviouslyPrivateCollectionsEndpoint = discprov => `${discprov}/previously_private/playlist`
...
// Returns the created at field of the last index retrievable
const getPreviousCreatedAt = async (route, startId) => {
  let id = startId
  while (id > 0) {
    const previousUrl = `${route}${id}`
    const { data: item } = await axios.get(previousUrl)
    if (item.error || !item.data || !item.data.length) {
      id -= 1
    } else {
      return item.data[0].created_at
    }
  }
}

// Gets all tracks/playlists that were previously private before the start id and made public after
// Finds the last valid id's created at to query for previously hidden tracks/playlists
const fetchPreviouslyHidden = async (startId, endpoint, previouslyHiddenEndpoint) => {
  const items = []
  try {
    const createdAt = await getPreviousCreatedAt(endpoint, startId)
    if (!createdAt) {
      throw new Error('Not found')
    }
    const url = `${previouslyHiddenEndpoint}?date=${encodeURIComponent(createdAt)}`

    const { data: { data: { ids } } } = await axios.get(url)

    if (!Array.isArray(ids)) {
      throw new Error('No previously unlisted')
    }
    for (const id of ids) {
    ...
    console.error(`Error on fetching previous from ${startId}: ${e.message}`)
    return []
  }
}
...
const fetchTracks = async (latestId, maxIterations) => {
  ...
  if (items.length > 0) {
    const previouslyUnlistedTracks = await fetchPreviouslyHidden(
      latestId,
      getTracksEndpoint(discoveryProvider),
      getPreviouslyUnlistedTracksEndpoint(discoveryProvider)
    )
    items.push(...previouslyUnlistedTracks)
  }
...
const fetchCollections = async (latestId, maxIterations) => {
  ...
  if (items.length > 0) {
    const previouslyUnlistedTracks = await fetchPreviouslyHidden(
      latestId,
      getCollectionsEndpoint(discoveryProvider),
      getPreviouslyPrivateCollectionsEndpoint(discoveryProvider)
    )
    items.push(...previouslyUnlistedTracks)
  }

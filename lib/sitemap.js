...
const createRootIndexSkeleton = (tracksIndex, collectionsIndex, usersIndex) => {
  const xml = create(VERSION)
  const sitemapindex = xml.ele('sitemapindex')
  sitemapindex.att('xmlns', XMLNS)
 ...
 */
const createRootSiteIndex = async (path, tracksIndex, collectionsIndex, usersIndex) => {
  const index = await createOrReadXMLAsObject(
    path,
    () => createRootIndexSkeleton(tracksIndex, collectionsIndex, usersIndex)
  )
 ...
 * @param {*} siteMapRoot the root directory to index all the files of
 * @param {*} latestNumber the last latest numbered xml file
 * @param {*} newNumber the new latest numbered xml file
 */
